# 📱 Londa Rides API - Complete Documentation
...
4. [User Authentication APIs](#1-user-authentication-apis)
   ...
   - 1.3 User Login
   - 1.4 Request Email OTP
   - 1.5 Verify Email OTP
   - 1.6 Create User Account
   - 1.7 Refresh Token
...
15. [Frontend/Mobile App Integration](#12-frontendmobile-app-integration)
...
3. API returns `accessToken` (Firebase Custom Token) in the response
4. **Exchange custom token for ID token** using Firebase SDK on the client side
5. Use the ID token for all subsequent authenticated requests

**⚠️ Important:** 
- The `accessToken` returned by `/verify-otp` and `/driver/verify-otp` is a **Firebase Custom Token**
- You MUST exchange it for an ID token using Firebase SDK before making authenticated requests
- See [Frontend Integration Guide](./FRONTEND_INTEGRATION_GUIDE.md) for complete code examples

**Security Note:** 
- `user_id` and `driver_id` are automatically extracted from the authentication token
...
**Authentication:** Not required  
**Description:** User login with OTP (same as verify-otp, different endpoint name for consistency with driver endpoints).
...
**Authentication:** Required (Bearer token)  
**Description:** Creates a new ride request. The `user_id` is automatically obtained from the authentication token. **Do NOT include `user_id` in the request body.**
...
**⚠️ Security Note:** The `user_id` is automatically extracted from the authentication token. Including it in the request body is not required and will be ignored.
...
**Authentication:** Required (Bearer token)  
**Description:** Finds available drivers near the user's location using Haversine distance calculation. Returns drivers within the specified radius, sorted by distance (closest first).
...
{
  "success": true,
  "message": "No drivers available within 5.0km radius",
  "data": {
    "drivers": [],
    "count": 0,
    "radius_km": 5.0
...
| Field | Type | Description |
|-------|------|-------------|
| `drivers` | array | List of nearby drivers sorted by distance |
| `drivers[].distance_km` | float | Distance from search location in kilometers |
| `drivers[].location` | object | Driver's current location (properly serialized) |
| `drivers[].location.latitude` | float | Driver's latitude |
| `drivers[].location.longitude` | float | Driver's longitude |
| `count` | integer | Number of drivers found |
| `radius_km` | float | Search radius used |
| `search_location` | object | The location that was searched from |
...
- Filters drivers by specified radius (only returns drivers within range)
- Results are **sorted by distance** (closest driver first)
- Each driver includes a `distance_km` field showing exact distance
- Location data is properly serialized (no GeoPoint objects in response)
- Only returns drivers with status "online"
- Maximum radius is 100km

#### Notes

- Drivers without valid location data are excluded from results
- Distance calculation accounts for Earth's curvature
- Response includes search parameters for client-side validation
- Empty results return success response with empty array (not an error)
...
**Authentication:** Required (Bearer token)  
**Description:** Cancels a ride request. The `user_id` is automatically obtained from the authentication token. **Do NOT include `user_id` in the request body.**
...
**Authentication:** Required (Bearer token)  
**Description:** Rates a completed ride. The `user_id` is automatically obtained from the authentication token. **Do NOT include `user_id` in the request body.**
...
**Authentication:** Required (Bearer token - Driver)  
**Description:** Updates driver subscription settings. Drivers can only update their own subscription.
...
{
  "success": false,
  "message": "No active subscription found for driver driver_123",
  "error": {
    "code": "NOT_FOUND",
    "details": {}
  },
...
### 7.7 Cancel Driver Subscription

**Endpoint:** `DELETE /api/v1/driver/subscription/{driver_id}`  
**Authentication:** Required (Bearer token - Driver)  
**Description:** Cancels a driver's active subscription. Drivers can only cancel their own subscription.
...
```json
{
  ...
  "data": {
    ...
    "startDate": "2024-12-31T19:00:00.000000",
    "endDate": "2025-01-30T19:00:00.000000",
    "cancellationReason": "No longer driving for Londa Rides",
    "updatedAt": "2024-12-31T19:10:00.000000"
  },
  "timestamp": "2024-12-31T19:10:00.000000"
}
...
**Authentication:** Required (Bearer token - Driver)  
**Description:** Gets earnings analytics for the logged-in driver with daily, weekly, and monthly breakdowns.
...
| Field | Type | Description |
|-------|------|-------------|
| `totalEarnings` | float | Total earnings from all completed rides (NAD) |
| `totalRides` | integer | Total number of completed rides |
| `averageEarningPerRide` | float | Average earning per ride (NAD) |
| `currency` | string | Currency code (NAD) |
| `daily` | object | Today's earnings summary and breakdown |
| `daily.total` | float | Total earnings for today (NAD) |
| `daily.rides` | integer | Number of rides completed today |
| `daily.breakdown` | array | Daily breakdown (single day) |
| `weekly` | object | Last 7 days earnings summary and breakdown |
| `weekly.total` | float | Total earnings for last 7 days (NAD) |
| `weekly.rides` | integer | Number of rides completed in last 7 days |
| `weekly.breakdown` | array | Daily breakdown for last 7 days |
| `monthly` | object | Last 30 days earnings summary and breakdown |
| `monthly.total` | float | Total earnings for last 30 days (NAD) |
| `monthly.rides` | integer | Number of rides completed in last 30 days |
| `monthly.breakdown` | array | Daily breakdown for last 30 days |

**Breakdown Array Fields:**

| Field | Type | Description |
|-------|------|-------------|
| `date` | string | Date in YYYY-MM-DD format |
| `earnings` | float | Total earnings for that date (NAD) |
| `rides` | integer | Number of rides completed on that date |

**Notes:**
- All breakdowns are sorted by date in descending order (most recent first)
- Daily period includes rides from 00:00:00 UTC today
- Weekly period includes rides from last 7 days
- Monthly period includes rides from last 30 days
...
1. **Register/Login:**
   - `POST /api/v1/registration` → Get `sessionInfo`
   - `POST /api/v1/verify-otp` → Get `accessToken`

2. **Create Account:**
   - `POST /api/v1/create-account` → Complete profile

3. **Use Token:**
   - Include `Authorization: Bearer <token>` in all protected requests

4. **Refresh Token (when expired):**
   - `POST /api/v1/refresh-token` → Get new `accessToken` (works with expired tokens)
...
## 12. Frontend/Mobile App Integration

### Overview

This section provides guidance for integrating the Londa Rides API into mobile and web applications. For complete integration examples, see the [Frontend Integration Guide](./FRONTEND_INTEGRATION_GUIDE.md).

### Quick Start

1. **Setup API Client** - Configure base URL and HTTP client
2. **Implement Authentication Flow** - Register, verify OTP, exchange custom token for ID token
3. **Add Request Interceptors** - Automatically add auth tokens to requests
4. **Add Response Interceptors** - Handle token expiration and errors
...
2. POST /api/v1/verify-otp → Get accessToken (Firebase Custom Token)
3. Exchange custom token for ID token (Firebase SDK - client-side)
4. Use ID token for all authenticated requests
5. Refresh token when expired (use /api/v1/refresh-token or Firebase SDK)
```

### Platform-Specific Guides

- **React Native**: See [Frontend Integration Guide](./FRONTEND_INTEGRATION_GUIDE.md#react-native-javascripttypescript)
- **Flutter**: See [Frontend Integration Guide](./FRONTEND_INTEGRATION_GUIDE.md#flutter-dart)
- **iOS (Swift)**: See [Frontend Integration Guide](./FRONTEND_INTEGRATION_GUIDE.md#ios-swift)
- **Android (Kotlin)**: See [Frontend Integration Guide](./FRONTEND_INTEGRATION_GUIDE.md#android-kotlin)
- **Web (JavaScript)**: See [Frontend Integration Guide](./FRONTEND_INTEGRATION_GUIDE.md#web-javascripttypescript)
...
Parse the `data` field for actual response content.

#### 4. Error Handling

Handle these HTTP status codes:
- **401 Unauthorized**: Token expired or invalid → Refresh token and retry
- **403 Forbidden**: Insufficient permissions → Check user type (user vs driver)
...
- ✅ Extract `user_id`/`driver_id` from token (never from request body)
- ✅ Use HTTPS in production
- ✅ Never log tokens in production
- ✅ Implement request/response interceptors for automatic token handling
...
// Request a ride (user_id is automatically extracted from token)
const response = await apiClient.post('/request-ride', {
  pickup_location: {
    latitude: -22.5700,
    longitude: 17.0836,
    name: "Pickup Point",
...
// Note: user_id is NOT included in the request body
// It is automatically extracted from the Authorization token
```

### Additional Resources

- [Frontend Integration Guide](./FRONTEND_INTEGRATION_GUIDE.md) - Complete integration examples for all platforms
...
- See [Frontend Integration Guide](./FRONTEND_INTEGRATION_GUIDE.md) for mobile app integration
...
## 13. Safety & Security APIs

### 13.1 Trigger SOS Alert
* **Endpoint**: `POST /ride/{ride_id}/sos`
* **Description**: Trigger an emergency alert for an active ride.
* **Authentication**: Required (User or Driver)
* **Request Body**:
  ```json
  {
    "location": {
...
### 13.2 Share Track Link
* **Endpoint**: `POST /ride/{ride_id}/share-tracking`
* **Description**: Generate a shareable tracking link to monitor ride progress.
...
## 14. Advanced Carpooling & Routing

### 14.1 Update Ride Stops
* **Endpoint**: `PUT /ride/{ride_id}/stops`
* **Description**: Add or update intermediate stops for a ride.
* **Authentication**: Required (User only)
* **Request Body**:
  ```json
  {
    "stops": [
      {
         "latitude": -22.57,
         "longitude": 17.07,
         "address": "Intermediate Stop Address"
      }
    ]
  }
  ```

### 14.2 Driver Route Optimization
* **Endpoint**: `GET /driver/route-optimization`
* **Description**: Get an optimized pickup/drop-off sequence based on accepted carpool rides and stops.
* **Authentication**: Required (Driver only)

## 15. Driver Vehicle & Documents

### 15.1 Get Vehicle Details
* **Endpoint**: `GET /driver/vehicle`
* **Description**: Get current driver's vehicle information.
* **Authentication**: Required (Driver only)

### 15.2 Update Vehicle Details
* **Endpoint**: `PUT /driver/vehicle`
* **Description**: Update current driver's vehicle information.
* **Authentication**: Required (Driver only)
* **Request Body**:
  ```json
  {
    ...
    "year": 2020,
    "color": "White",
    "licensePlate": "N 12345 W",
    "capacity": 4
  }
  ```

### 15.3 Upload Document Metadata
* **Endpoint**: `POST /driver/documents/metadata`
* **Description**: Upload metadata (like Cloudinary URL) for a driver document.
* **Authentication**: Required (Driver only)
* **Request Body**:
  ```json
  {
